...
namespace crypto_test {
    class MD5Hash {
        ...
        public void WindowHashing(object sender, EventArgs args) {
            string curExt = WordPad.Helpers.ReadHelper.GetExtention(_form.Text);
            bool isText = false;
            _progress = new Utils.Progress(0, 2, 1, "Hash progress");

            if (WordPad.Form1.TxtExts.Any(str => str == curExt)) {
                isText = true;
            }

            string ans = GetHash(_textBox.Text, isText);
            _progress.CloseFormSafe();
            Utils.TextForm textForm = new Utils.TextForm(ans);
            textForm.Show();
